...
//        helpY[Nx] = y[Ny - 1] - stepY / 2;
        ...
        for (j = 1; j < Ny; j++) {
            ...
            f[j][0] = g1(y[j], type);
            for (int i = 1; i < Nx; i++) {
                a[j][i][i] = -(hx * k2(x[i], helpY[j - 1], type)) / hy;     // коэффициент i j - 1
            ...
            // правая граница
            I = Nx;
            a[j][I][I] = -(hx / 2 * k2(x[I], helpY[j - 1], type)) / hy;     // коэффициент i j - 1
            c[j][I][I - 1] = -(hy * k1(helpX[I - 1], y[j], type)) / hx; // i-1, j
    ...
    public static double[][] solveSystem(double[][][] massA, double[][][] massB, double[][][] massC, double[][] func, int x, int y) {
        ...
        tempResult = inversion(massC[0], x);
        double[][] tempRes2 = minusOne(tempResult);
        alpha[0] = multiplicationOfMatrix(tempRes2, massB[0]);

        tempBeta = inversion(massC[0], x);
        beta[0] = multiplicationMatrixAndVector(tempBeta, func[0]);

        for (int i = 1; i < y; i++) {
            double[][] ttt = multiplicationOfMatrix(massA[i - 1], alpha[i - 1]);
            double[][] tempResCom = plusMatrix(ttt, massC[i - 1]);
            double[][] tempResCom1 = inversion(tempResCom, x);
            double[][] tempRes1 = minusOne(tempResCom1);
            alpha[i] = multiplicationOfMatrix(tempRes1, massB[i - 1]);
    ...
    static void printAllMatrix(double[][][] tempMatrix) {
        for (int j = 0; j < Ny; j++) {
            for (int i = 0; i < Nx; i++) {
                for (int k = 0; k < Nx; k++) {
                    System.out.print(tempMatrix[j][i][k] + " ");
                }
                System.out.println();
            }
            System.out.println("/------------------------------/");
    ...
    public static double[] minusVector(double[] matrix1, double[] matrix2) {
    ...
    public static double[] plusVector(double[] matrix1, double[] matrix2) {
        double[] res = new double[matrix1.length];
        for (int i = 0; i < matrix1.length; ++i) {
            res[i] = matrix1[i] + matrix2[i];
        }
    ...
    public static void printMatrix(double[][] matrix) {
        for (double[] elemOfMatrix : matrix) {
            for (int j = 0; j < matrix[0].length; ++j) {
                System.out.print(elemOfMatrix[j] + " ");
            }
            System.out.println();
        }
    }
